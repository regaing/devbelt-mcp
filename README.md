...
- **文本转换**：简繁互转、拼音、全角半角、竖排、去重、火星文、UUID 生成、身份证解析等 15 种
- **单位换算**：长度/面积/温度/速度/压力/数据大小等 13 类
- **网络查询**：WHOIS、ICP 备案、IP 归属、DNS 解析、HTTP 状态、Gzip 检测、死链检测、网页抓取、WebSocket 完整会话测试
- **时间工具**：时间戳互转、多时区对照、时间差/倒计时、cron 解析、时长换算、strftime 格式化
- **其他**：二维码/条形码/ICO 图标生成、万年历、17 类参考表查询
