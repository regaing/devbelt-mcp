/**
 * 身份证解析（GB 11643-1999，纯本地算法，无三方接口）
 *
 * - 18 位校验码算法：加权因子 [7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2]，校验码表 "10X98765432"
 * - 15 位 → 18 位升级：补 "19" + 出生日期，计算校验位
 * - 行政区划：省级（34 条准确）+ 地级（约 340 条）
 */
...
/** 地级行政区划码表（前 4 位，含省直辖县级） */
const CITIES: Record<string, string> = {
  "1101": "北京市辖区", "1201": "天津市辖区", "3101": "上海市辖区", "5001": "重庆市辖区",
  "1301": "石家庄市", "1302": "唐山市", "1303": "秦皇岛市", "1304": "邯郸市", "1305": "邢台市",
  "1306": "保定市", "1307": "张家口市", "1308": "承德市", "1309": "沧州市", "1310": "廊坊市", "1311": "衡水市",
  "1401": "太原市", "1402": "大同市", "1403": "阳泉市", "1404": "长治市", "1405": "晋城市", "1406": "朔州市",
  "1407": "晋中市", "1408": "运城市", "1409": "忻州市", "1410": "临汾市", "1411": "吕梁市",
  ...
  "6501": "乌鲁木齐市", "6502": "克拉玛依市", "6504": "吐鲁番市", "6505": "哈密市", "6523": "昌吉回族自治州",
  "6527": "博尔塔拉蒙古自治州", "6528": "巴音郭楞蒙古自治州", "6529": "阿克苏地区", "6530": "克孜勒苏柯尔克孜自治州",
  "6531": "喀什地区", "6532": "和田地区", "6540": "伊犁哈萨克自治州", "6542": "塔城地区", "6543": "阿勒泰地区",
  "6590": "自治区直辖县级行政区划",
  "7100": "台湾省", "8100": "香港特别行政区", "8200": "澳门特别行政区",
};
...
export function calcCheckCode(id17: string): string {
  let sum = 0;
  for (let i = 0; i < 17; i++) sum += parseInt(id17[i], 10) * WEIGHTS[i];
  return CHECK_CODES[sum % 11];
}

/** 15 位 → 18 位 */
export function upgrade15to18(id15: string): string {
  const id17 = id15.slice(0, 6) + "19" + id15.slice(6);
  return id17 + calcCheckCode(id17);
}
...
/** 身份证解析（18/15 位） */
export function parseIdCard(input: string): IdCardResult {
  const id = input.trim().toUpperCase();
  if (!/^\d{17}[\dX]$/.test(id) && !/^\d{15}$/.test(id)) {
    return { valid: false, error: "格式无效（需 15 位数字或 18 位数字+X）" };
  }

  let id18 = id;
  let sourceLength = id.length;
  if (id.length === 15) {
    id18 = upgrade15to18(id);
  }

  const province = PROVINCES[id18.slice(0, 2)] ?? "未知";
  const city = CITIES[id18.slice(0, 4)] ?? (PROVINCES[id18.slice(0, 2)] ? "未知地级" : "未知");
  const birthStr = `${id18.slice(6, 10)}-${id18.slice(10, 12)}-${id18.slice(12, 14)}`;
  ...
  if (
    ...
    return { valid: false, error: `出生日期无效：${birthStr}` };
  }
  if (year < 1900 || year > new Date().getFullYear()) {
    return { valid: false, error: `出生年份异常：${year}` };
  }

  const checkDigit = id18[17];
  const checkPass = calcCheckCode(id18.slice(0, 17)) === checkDigit;
  const gender = parseInt(id18[16], 10) % 2 === 1 ? "男" : "女";
